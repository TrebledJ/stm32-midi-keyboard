...
#include "leaf.h"
#include "leaf/leaf-oscillators.h"
...
// Cycle
void tCycle_init(tCycle* const cy, LEAF* const leaf) { tCycle_initToPool(cy, &leaf->mempool); }
...
//========================================================================
/* Triangle */
void tTriangle_init(tTriangle* const cy, LEAF* const leaf) { tTriangle_initToPool(cy, &leaf->mempool); }
...
float tTriangle_tick(tTriangle* const cy)
{
    ...
    frac              = (float)tempFrac
           * 0.000000476837386f; // 1/2097151 (2097151 is the 21 bits after the 11 bits that represent the main index)
...
//========================================================================
/* Square */
void tSquare_init(tSquare* const cy, LEAF* const leaf) { tSquare_initToPool(cy, &leaf->mempool); }
...
//=====================================================================
// Sawtooth
void tSawtooth_init(tSawtooth* const cy, LEAF* const leaf) { tSawtooth_initToPool(cy, &leaf->mempool); }
...
void tPBTriangle_init(tPBTriangle* const osc, LEAF* const leaf) { tPBTriangle_initToPool(osc, &leaf->mempool); }
...
void tPBPulse_init(tPBPulse* const osc, LEAF* const leaf) { tPBPulse_initToPool(osc, &leaf->mempool); }
...
void tPBSaw_init(tPBSaw* const osc, LEAF* const leaf) { tPBSaw_initToPool(osc, &leaf->mempool); }
...
void tPhasor_init(tPhasor* const ph, LEAF* const leaf) { tPhasor_initToPool(ph, &leaf->mempool); }
...
//========================================================================
/* Noise */
void tNoise_init(tNoise* const ns, NoiseType type, LEAF* const leaf) { tNoise_initToPool(ns, type, &leaf->mempool); }
...
void tNeuron_init(tNeuron* const nr, LEAF* const leaf) { tNeuron_initToPool(nr, &leaf->mempool); }
...
float tNeuron_tick(tNeuron* const nr)
{
    ...
    n->voltage =
        voltage + (n->timeStep * n->current / n->C)
        - (n->timeStep
           * (n->rate[0] * (voltage - n->V[0]) + n->rate[1] * (voltage - n->V[1]) + n->rate[2] * (voltage - n->V[2])));
...
void tMBPulse_init(tMBPulse* const osc, LEAF* const leaf) { tMBPulse_initToPool(osc, &leaf->mempool); }
...
void tMBTriangle_init(tMBTriangle* const osc, LEAF* const leaf) { tMBTriangle_initToPool(osc, &leaf->mempool); }
...
void tMBSaw_init(tMBSaw* const osc, LEAF* const leaf) { tMBSaw_initToPool(osc, &leaf->mempool); }
...
void tMBSawPulse_init(tMBSawPulse* const osc, LEAF* const leaf) { tMBSawPulse_initToPool(osc, &leaf->mempool); }
...
#endif
{
    ...
    else if (!k) { /* normal operation for pulse, signal currently high */
...
#ifdef ITCMRAM
float __attribute__((section(".itcmram"))) __attribute__((aligned(32)))
tMBSawPulse_sync(tMBSawPulse* const osc, float value)
#else
...
float tWaveOsc_tick(tWaveOsc* const cy)
{
    ...
    // Phasor increment (unsigned 32bit int wraps automatically with overflow so no need for if branch checks, as you
    // need with float)
    c->phase += c->inc;
...
void tWaveOsc_setFreq(tWaveOsc* const cy, float freq)
{
    ...
    c->oct = (int)c->w;
    c->w -= c->oct;
    if (c->oct >= c->numSubTables - 1)
        c->oct = c->numSubTables - 2;
}
...
// Cycle
void tIntPhasor_init(tIntPhasor* const cy, LEAF* const leaf) { tIntPhasor_initToPool(cy, &leaf->mempool); }
...
//////SQUARE(PUSHER)LFO
void tSquareLFO_init(tSquareLFO* const cy, LEAF* const leaf) { tSquareLFO_initToPool(cy, &leaf->mempool); }
...
void tSawSquareLFO_init(tSawSquareLFO* const cy, LEAF* const leaf) { tSawSquareLFO_initToPool(cy, &leaf->mempool); }
...
/// tri
void tTriLFO_init(tTriLFO* const cy, LEAF* const leaf) { tTriLFO_initToPool(cy, &leaf->mempool); }
...
float tTriLFO_tick(tTriLFO* const cy)
{
    ...
    // bitmask fun
    int32_t shiftedPhase =
        c->phase
        + 1073741824; // offset by 1/4" wave by adding 2^30 to get things in phase with the other LFO oscillators
...
void tSineTriLFO_init(tSineTriLFO* const cy, LEAF* const leaf) { tSineTriLFO_initToPool(cy, &leaf->mempool); }
