...
/* ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ NReverb ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ */
void tNReverb_init(tNReverb* const rev, float t60, LEAF* const leaf) { tNReverb_initToPool(rev, t60, &leaf->mempool); }
...
void tNReverb_initToPool(tNReverb* const rev, float t60, tMempool* const mp)
{
    ...
    int lengths[15] = {1433, 1601, 1867, 2053, 2251, 2399, 347, 113,
                       37,   59,   53,   43,   37,   29,   19}; // Delay lengths for 44100 Hz sample rate.
...
void tNReverb_setT60(tNReverb* const rev, float t60)
{
    ...
    for (int i = 0; i < 6; i++)
        r->combCoeffs[i] = powf(10.0f, (-3.0f * tLinearDelay_getDelay(&r->combDelays[i]) * r->invSampleRate / t60));
}
...
float tDattorroReverb_tick(tDattorroReverb* const rev, float input)
{
    ...
    // TAP OUT 1
    f1_sample = tTapeDelay_tapOut(&r->f1_delay_1, SAMP(8.9f)) + tTapeDelay_tapOut(&r->f1_delay_1, SAMP(99.8f));
    ...
    // TAP OUT 2
    f2_sample = tTapeDelay_tapOut(&r->f2_delay_1, SAMP(11.8f)) + tTapeDelay_tapOut(&r->f2_delay_1, SAMP(121.7f));
