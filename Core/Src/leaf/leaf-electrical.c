...
#include "leaf.h"
#include "leaf/leaf-electrical.h"
...
static void wdf_init(tWDF* const wdf, WDFComponentType type, float value, tWDF* const rL, tWDF* const rR)
{
    ...
    if (r->type == Resistor) {
        ...
        r->port_resistance_left  = tWDF_getPortResistance(r->child_left);
        r->port_resistance_right = tWDF_getPortResistance(r->child_right);
        r->port_resistance_up =
            (r->port_resistance_left * r->port_resistance_right) / (r->port_resistance_left + r->port_resistance_right);
        r->port_conductance_up    = 1.0f / r->port_resistance_up;
        r->port_conductance_left  = 1.0f / r->port_resistance_left;
        r->port_conductance_right = 1.0f / r->port_resistance_right;
        r->gamma_zero             = 1.0f / (r->port_resistance_right + r->port_resistance_left);
...
void tWDF_initToPool(tWDF* const wdf, WDFComponentType type, float value, tWDF* const rL, tWDF* const rR,
                     tMempool* const mp)
{
    _tMempool* m = *mp;
...
{
    _tWDF* r = *wdf;
    if (r->child_left == NULL && r->child_right == NULL)
        return 1;
    return 0;
}
...
static void set_incident_wave_for_series(tWDF* const wdf, float incident_wave, float input)
{
    ...
    tWDF_setIncidentWave(r->child_left,
                         (-1.0f * gamma_left * incident_wave) + (gamma_right * left_wave) - (gamma_left * right_wave),
                         input);
    tWDF_setIncidentWave(r->child_right,
                         (-1.0f * gamma_right * incident_wave) + (gamma_left * right_wave) - (gamma_right * left_wave),
                         input);
    // From rt-wdf
    //  tWDF_setIncidentWave(r->child_left, gamma_left * (left_wave * ((1.0f / gamma_left) - 1.0f) - right_wave -
    //  incident_wave)); tWDF_setIncidentWave(r->child_right, gamma_right * (right_wave * ((1.0f / gamma_right) - 1.0f)
    //  - left_wave - incident_wave));
}
...
static float get_reflected_wave_for_series(tWDF* const wdf, float input)
{
    ...
    r->reflected_wave_up =
        (-1.0f * (tWDF_getReflectedWaveUp(r->child_left, input) + tWDF_getReflectedWaveUp(r->child_right, input)));
    return r->reflected_wave_up;
}
...
static float get_reflected_wave_for_parallel(tWDF* const wdf, float input)
{
    ...
    r->reflected_wave_up = (gamma_left * tWDF_getReflectedWaveUp(r->child_left, input)
                            + gamma_right * tWDF_getReflectedWaveUp(r->child_right, input));
    return r->reflected_wave_up;
}
