...
#include "leaf.h"
#include "leaf/leaf-envelopes.h"
#include "leaf/leaf-tables.h"
...
float tEnvelope_tick(tEnvelope* const envlp)
{
    ...
    if (env->inAttack) {
        ...
        if (env->attackPhase > UINT16_MAX) {
            ...
            env->next =
                env->gain
                * env->exp_buff[UINT16_MAX - (uint32_t)env->attackPhase]; // inverted and backwards to get proper rising
                                                                          // exponential shape/perception
        }
...
float tADSR_tick(tADSR* const adsrenv)
{
    ...
    if (adsr->inAttack) {
        ...
        if (adsr->attackPhase > UINT16_MAX) {
            ...
            adsr->next =
                adsr->gain
                * adsr->exp_buff[UINT16_MAX - (uint32_t)adsr->attackPhase]; // inverted and backwards to get proper
                                                                            // rising exponential shape/perception
        }
    ...
    if (adsr->inDecay) {
        ...
        if (adsr->decayPhase >= UINT16_MAX) {
            ...
            adsr->next =
                (adsr->gain * (adsr->sustain + ((adsr->exp_buff[(uint32_t)adsr->decayPhase]) * (1.0f - adsr->sustain))))
...
void tADSRS_initToPool(tADSRS* const adsrenv, float attack, float decay, float sustain, float release,
                       tMempool* const mp)
{
    _tMempool* m  = *mp;
...
float tADSRS_tick(tADSRS* const adsrenv)
{
    ...
    switch (adsr->state) {
        ...
        case env_decay:
            ...
            break;
        case env_sustain: adsr->output = adsr->output * adsr->leakFactor; break;
        case env_release:
    ...
    // smooth the gain value   -- this is not ideal, a retrigger while the envelope is still going with a new gain will
    // cause a jump, although it will be smoothed quickly. Maybe doing the math so the range is computed based on the
    // gain rather than 0.->1. is preferable? But that's harder to get the exponential curve right without a lookup.
...
void tADSRT_init(tADSRT* const adsrenv, float attack, float decay, float sustain, float release, float* expBuffer,
                 int bufferSize, LEAF* const leaf)
{
    tADSRT_initToPool(adsrenv, attack, decay, sustain, release, expBuffer, bufferSize, &leaf->mempool);
}

// initialize with an exponential function that decays -- i.e. a call to LEAF_generate_exp(expBuffer, 0.001f,
// 0.0f, 1.0f, -0.0008f, EXP_BUFFER_SIZE); times are in ms
void tADSRT_initToPool(tADSRT* const adsrenv, float attack, float decay, float sustain, float release, float* expBuffer,
                       int bufferSize, tMempool* const mp)
{
    _tMempool* m  = *mp;
    ...
    switch (adsr->whichStage) {
        case env_ramp:
            if (adsr->rampPhase > adsr->buff_sizeMinusOne) {
                ...
                adsr->next =
                    adsr->rampPeak * LEAF_interpolation_linear(adsr->exp_buff[intPart], secondValue, floatPart);
            }
                ...
                adsr->next =
                    ...
                    * (1.0f
                       - LEAF_interpolation_linear(
                           adsr->exp_buff[intPart], secondValue,
                           floatPart)); // inverted and backwards to get proper rising exponential shape/perception
            }
        ...
        case env_sustain: adsr->next = adsr->next * adsr->leakFactor; break;
        ...
        case env_release:
            ...
            if (adsr->releasePhase > adsr->buff_sizeMinusOne) {
                ...
                adsr->next = adsr->releasePeak
                             * (LEAF_interpolation_linear(adsr->exp_buff[intPart], secondValue,
                                                          floatPart)); // do interpolation !
            }
    ...
    switch (adsr->whichStage) {
        ...
        case env_attack:
            ...
            if (adsr->attackPhase > adsr->buff_sizeMinusOne) {
                ...
                adsr->next =
                    adsr->gain
                    * (1.0f - adsr->exp_buff[(uint32_t)adsr->attackPhase]); // inverted and backwards to get proper
                                                                            // rising exponential shape/perception
            }
...
void tRampUpDown_initToPool(tRampUpDown* const r, float upTime, float downTime, int samples_per_tick,
                            tMempool* const mp)
{
    _tMempool* m       = *mp;
...
{ // factor is usually a value between 0 and 0.1. Lower value is slower. 0.01 for example gives you a smoothing time of
  // about 10ms
