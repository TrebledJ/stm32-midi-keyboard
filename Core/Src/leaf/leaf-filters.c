...
// ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ OnePole Filter ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ //
void tOnePole_init(tOnePole* const ft, float freq, LEAF* const leaf) { tOnePole_initToPool(ft, freq, &leaf->mempool); }
...
// ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ TwoPole Filter ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ //
void tTwoPole_init(tTwoPole* const ft, LEAF* const leaf) { tTwoPole_initToPool(ft, &leaf->mempool); }
...
void tTwoPole_setResonance(tTwoPole* const ft, float frequency, float radius, int normalize)
{
    ...
    if (frequency < 0.0f)
        frequency = 0.0f;
    if (frequency > (sampleRate * 0.49f))
        frequency = sampleRate * 0.49f;
    if (radius < 0.0f)
        radius = 0.0f;
    if (radius >= 1.0f)
        radius = 0.999999f;
...
// ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ TwoZero Filter ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ //
void tTwoZero_init(tTwoZero* const ft, LEAF* const leaf) { tTwoZero_initToPool(ft, &leaf->mempool); }
...
// ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ PoleZero Filter ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ //
void tPoleZero_init(tPoleZero* const pzf, LEAF* const leaf) { tPoleZero_initToPool(pzf, &leaf->mempool); }
...
// ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ BiQuad Filter ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ //
void tBiQuad_init(tBiQuad* const ft, LEAF* const leaf) { tBiQuad_initToPool(ft, &leaf->mempool); }
...
void tButterworth_setF1(tButterworth* const ft, float f1)
{
    ...
    if (f->f1 < 0.0f || f1 < 0.0f)
        return;

    f->f1 = f1;
    for (int i = 0; i < f->order; ++i)
        tSVF_setFreq(&f->svf[i], f1);
}
...
float tFIR_tick(tFIR* const firf, float input)
{
    ...
    float y      = 0.0f;
    for (int i = 0; i < fir->numTaps; ++i)
        y += fir->past[i] * fir->coeff[i];
    for (int i = fir->numTaps - 1; i > 0; --i)
        fir->past[i] = fir->past[i - 1];
    return y;
}
...
/// translated from a Gen~ port of the Supercollider code that I believe was made by Rodrigo Costanzo and which I got
/// from PA Tremblay - JS
...
float tVZFilter_tick(tVZFilter* const vf, float in)
{
    ...
    // if we wanted to go into self-oscillation with stabiity we would need to use an anti-saturator on the feedback
    // loop, haven't figured that out yet. -JS
...
void tVZFilter_calcCoeffs(tVZFilter* const vf)
{
    ...
    switch (f->type) {
        ...
        case Morph: {
            ...
            float s =
                f->G
                * fastsqrtf((f->R2 * f->R2) / (f->cL * f->cL + f->cB * f->cB + f->cH * f->cH - 2.0f * f->cL * f->cH))
                * 2.0f;
            f->cL *= s;
            f->cB *= s;
            f->cH *= s;
...
float tDiodeFilter_tick(tDiodeFilter* const vf, float in)
{
    ...
    float y3 = (f->s2 + f->s3 + t2 * (f->s1 + f->s2 + f->s3 + t1 * (f->s0 + f->s1 + f->s2 + f->s3 + t0 * in))
                + t1 * (2.0f * f->s2 + 2.0f * f->s3))
                   * t3
               + f->s3 + 2.0f * f->s3 * t1 + t2 * (2.0f * f->s3 + 3.0f * f->s3 * t1);
#ifdef SAFE_FILTER
    if (isnan(y3)) {
        errorCheck = 1;
    }
#endif
    float tempy3denom = (t4 + t1 * (2.0f * t4 + 4.0f) + t2 * (t4 + t1 * (t4 + f->r * t0 + 4.0f) + 3.0f) + 2.0f) * t3
                        + t4 + t1 * (2.0f * t4 + 2.0f) + t2 * (2.0f * t4 + t1 * (3.0f * t4 + 3.0f) + 2.0f) + 1.0f;
#ifdef SAFE_FILTER
...
{
    // possible speedup with sqrt CMSIS-DSP approximation? seems to resolve to just a normal call to sqrt. Maybe the
    // vector version since there are two square roots to determine? -JS
    //	#ifdef ARM_MATH_CM7
    //		float output1;
    //		float output2;
...
float tanhd(const float x, const float d, const float s) { return 1.0f - s * (d + 1.0f) * x * x / (d + x * x); }
...
float tLadderFilter_tick(tLadderFilter* const vf, float in)
{
    ...
    for (int i = 0; i < f->oversampling; i++) {
        ...
        float estimate = g3 * f->b[3] + f3 * g2 * f->b[2] + f2 * g1 * f->b[1] + f1 * g0 * f->b[0] + f0 * in;
