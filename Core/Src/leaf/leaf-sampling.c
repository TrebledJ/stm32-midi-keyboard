...
{
    _tBuffer* s = *sb;
    if ((idx < 0) || (idx >= (int)s->bufferLength))
        return 0.f;
    return s->buff[idx];
}
...
void tSampler_initToPool(tSampler* const sp, tBuffer* const b, tMempool* const mp, LEAF* const leaf)
{
    ...
    p->idx     = 0.f;
    float rate = p->rateFactor; // adjust for sampling rate of buffer (may be different from leaf.sampleRate if audio
                                // file was loaded form SD card)
    if (rate < 0.f) {
        rate   = -rate;
        p->dir = -1;
    } else {
...
float tSampler_tick(tSampler* const sp)
{
    ...
    sample = LEAF_interpolate_hermite_x(buff[i1], buff[i2], buff[i3], buff[i4], alpha);

    int32_t cfxlen = p->cfxlen;
    if (p->len * 0.25f < cfxlen)
        cfxlen = p->len * 0.25f;

    // Determine crossfade points
    int32_t fadeLeftStart = 0;
    if (myStart >= cfxlen)
        fadeLeftStart = myStart - cfxlen;
    ...
    if (p->mode == PlayLoop) {
        ...
        if (p->inCrossfade) {
            ...
            cfxsample = LEAF_interpolate_hermite_x(buff[c1], buff[c2], buff[c3], buff[c4], alpha);
            if (cfxlen > 0.0f)
        ...
        if (p->flipIdx >= 0) {
            ...
            flipsample = LEAF_interpolate_hermite_x(buff[f1], buff[f2], buff[f3], buff[f4], falpha);
    ...
    sample =
        ((sample * (1.0f - crossfadeMix)) + (cfxsample * crossfadeMix)) * (1.0f - flipMix) + (flipsample * flipMix);
...
float tSampler_tickStereo(tSampler* const sp, float* outputArray)
{
    ...
    outputArray[0] = LEAF_interpolate_hermite_x(buff[i1 * p->channels], buff[i2 * p->channels], buff[i3 * p->channels],
                                                buff[i4 * p->channels], alpha);

    outputArray[1] = LEAF_interpolate_hermite_x(buff[(i1 * p->channels) + 1], buff[(i2 * p->channels) + 1],
                                                buff[(i3 * p->channels) + 1], buff[(i4 * p->channels) + 1], alpha);
    ...
    if (p->mode == PlayLoop) {
        ...
        if (p->inCrossfade) {
            ...
            cfxsample[0] = LEAF_interpolate_hermite_x(buff[c1 * p->channels], buff[c2 * p->channels],
                                                      buff[c3 * p->channels], buff[c4 * p->channels], alpha);

            cfxsample[1] =
                LEAF_interpolate_hermite_x(buff[(c1 * p->channels) + 1], buff[(c2 * p->channels) + 1],
                                           buff[(c3 * p->channels) + 1], buff[(c4 * p->channels) + 1], alpha);
        ...
        if (p->flipIdx >= 0) {
            ...
            flipsample[0] = LEAF_interpolate_hermite_x(buff[f1 * p->channels], buff[f2 * p->channels],
                                                       buff[f3 * p->channels], buff[f4 * p->channels], falpha);

            flipsample[1] =
                LEAF_interpolate_hermite_x(buff[(f1 * p->channels) + 1], buff[(f2 * p->channels) + 1],
                                           buff[(f3 * p->channels) + 1], buff[(f4 * p->channels) + 1], falpha);
    ...
    for (int i = 0; i < p->channels; i++) {
        outputArray[i] = ((outputArray[i] * (1.0f - crossfadeMix)) + (cfxsample[i] * crossfadeMix)) * (1.0f - flipMix)
                         + (flipsample[i] * flipMix);
...
static void handleStartEndChange(tSampler* const sp)
{
    ...
    if (p->cfxlen > (p->len * 0.25f))
        p->cfxlen = p->len * 0.25f;
...
void tSampler_setRate(tSampler* const sp, float rate)
{
    ...
    rate = rate * p->rateFactor; // adjust for sampling rate of buffer (may be different from leaf.sampleRate if audio
                                 // file was loaded form SD card)
    if (rate < 0.f) {
        rate   = -rate;
        p->dir = -1;
...
float tAutoSampler_tick(tAutoSampler* const as, float input)
{
    ...
    if ((currentPower > (a->threshold)) && (currentPower > a->previousPower + 0.001f) && (a->sampleTriggered == 0)
        && (a->sampleCounter == 0)) {
        a->sampleTriggered = 1;
        tBuffer_record(&a->sampler->samp);
        a->sampler->samp->recordedLength = a->sampler->samp->bufferLength;
        a->sampleCounter = a->windowSize + 24; // arbitrary extra time to avoid resampling while playing previous sample
                                               // - better solution would be alternating buffers and crossfading
        a->powerCounter = 1000;
    }
    ...
    if (w > 0.0f) {
        if (p >= (float)end) { /* normal phase reset */
            ...
            if (c->mode == PlayNormal) {
                c->active = 0;
            } else if (c->mode == PlayBackAndForth)
                w = -w;
        }
        ...
        //        }
            ...
            if ((end - p)
                < 480) // 480 samples should be enough to let the tExpSmooth go from 1 to 0 (10ms at 48k, 5ms at 192k)
                if (c->mode == PlayNormal) {
...
void tMBSampler_setEnd(tMBSampler* const sp, int32_t end)
{
    ...
    else if (end >= p->samp->bufferLength)
        p->end = p->samp->bufferLength - 1;
    else
        p->end = end;
}
