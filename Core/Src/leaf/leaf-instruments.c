...
#include "leaf/leaf-instruments.h"
...
float t808Cowbell_tick(t808Cowbell* const cowbellInst)
{
    ...
    // Mix oscillators.
    sample =
        (cowbell->oscMix * tSquare_tick(&cowbell->p[0])) + ((1.0f - cowbell->oscMix) * tSquare_tick(&cowbell->p[1]));
    ...
    if (cowbell->useStick)
        sample += (0.1f * tEnvelope_tick(&cowbell->envStick)
                   * tSVF_tick(&cowbell->bandpassStick, tNoise_tick(&cowbell->stick)));
...
void t808Hihat_init(t808Hihat* const hihatInst, LEAF* const leaf) { t808Hihat_initToPool(hihatInst, &leaf->mempool); }
...
float t808Hihat_tick(t808Hihat* const hihatInst)
{
    ...
    sample +=
        ((0.5f * tEnvelope_tick(&hihat->envStick)) * tSVF_tick(&hihat->bandpassStick, tNoise_tick(&hihat->stick)));
...
void t808Snare_init(t808Snare* const snareInst, LEAF* const leaf) { t808Snare_initToPool(snareInst, &leaf->mempool); }
...
float t808Snare_tick(t808Snare* const snareInst)
{
    ...
    float sample = (snare->toneNoiseMix) * (tone[0] * snare->toneGain[0] + tone[1] * snare->toneGain[1])
                   + (1.0f - snare->toneNoiseMix) * (noise * snare->noiseGain);
    sample = tanhf(sample * 2.0f);
    return sample;
}
...
void t808Kick_init(t808Kick* const kickInst, LEAF* const leaf) { t808Kick_initToPool(kickInst, &leaf->mempool); }
...
float t808Kick_tick(t808Kick* const kickInst)
{
    ...
    tCycle_setFreq(&kick->tone, (kick->toneInitialFreq
                                 * (1.0f + (kick->chirpRatioMinusOne * tEnvelope_tick(&kick->toneEnvOscChirp))))
                                    + (kick->sighAmountInHz * tEnvelope_tick(&kick->toneEnvOscSigh)));
