...
void tStack_init(tStack* const stack, LEAF* const leaf) { tStack_initToPool(stack, &leaf->mempool); }
...
int tStack_next(tStack* const stack)
{
    ...
    {
        if (ns->pos > 0) // if you're not at the most recent note (first one), then go backward in the array (moving
...
int tPoly_noteOn(tPoly* const polyh, int note, uint8_t vel)
{
    ...
    if (tStack_contains(&poly->stack, note) >= 0)
        return -1;
    else {
        ...
        if (!found) // steal
        {
            ...
            for (int j = tStack_getSize(&poly->stack) - 1; j >= 0; j--) {
                ...
                if (whichVoice >= 0) {
                    ...
                    poly->notes[oldNote][1] =
                        -1; // mark the stolen voice as inactive (in the second dimension of the notes array)
                    poly->notes[note][0] = vel;
    ...
    if (deactivatedVoice >= 0) {
        for (int j = 0; j < tStack_getSize(&poly->stack); ++j) {
            ...
            if (poly->notes[noteToTest][1] < 0) // if there is a stolen note waiting (marked inactive but on the stack)
            {
                ...
                poly->voices[deactivatedVoice][1] =
                    poly->notes[noteToTest][0]; // set the velocity of the voice to be the velocity of that note
    ...
    if (tStack_contains(&poly->stack, note) >= 0)
        ...
        for (int i = 0; i < poly->numVoices; i++) {
            if (poly->voices[i][0] == -1) // if inactive voice, give this note to voice
            {
                ...
                poly->voices[i][2] =
                    note; // voices[i][2] is the output midi note, (avoiding the -1 when a voice is inactive)
        ...
        if ((!found) && (poly->stealing_on)) // steal
        {
            for (int j = tStack_getSize(&poly->stack) - 1; j >= 0; j--) {
                ...
                if (whichVoice >= 0) {
                    ...
                    poly->voices[whichVoice][0] = note;
                    poly->voices[whichVoice][1] = vel;
                    poly->notes[oldNote][0] =
                        -3; // mark the stolen voice as stolen (in the second dimension of the notes array)
    ...
    if (poly->recover_stolen) {
        ...
        if (deactivatedVoice >= 0) {
            for (int j = 0; j < tStack_getSize(&poly->stack); ++j) {
                ...
                if (poly->notes[noteToTest][0]
                    == -3) // if there is a stolen note waiting (marked inactive but on the stack)
                {
                    poly->voices[deactivatedVoice][0] =
                        noteToTest; // set the newly free voice to use the old stolen note
                    poly->voices[deactivatedVoice][1] =
                        poly->notes[noteToTest][1]; // set the velocity of the voice to be the velocity of that note
...
void tSimplePoly_deactivateVoice(tSimplePoly* const polyh, uint8_t voice)
{
    ...
    if (poly->voices[voice][0]
        == -2) // only do this if the voice is waiting for deactivation (not already reassigned while waiting)
    {
