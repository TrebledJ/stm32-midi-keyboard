...
#include "leaf/leaf-distortion.h"
...
void tSampleReducer_init(tSampleReducer* const sr, LEAF* const leaf) { tSampleReducer_initToPool(sr, &leaf->mempool); }
...
void tOversampler_initToPool(tOversampler* const osr, int maxRatio, int extraQuality, tMempool* const mp)
{
    ...
    if (extraQuality)
        offset = 6;
    if (maxRatio == 2 || maxRatio == 4 || maxRatio == 8 || maxRatio == 16 || maxRatio == 32 || maxRatio == 64) {
        ...
        os->pCoeffs          = __leaf_tableref_firCoeffs[idx];
...
tOversampler_upsample(tOversampler* const osr, float input, float* output)
#else
...
tOversampler_downsample(tOversampler* const osr, float* input)
#else
...
void tOversampler_setRatio(tOversampler* const osr, int ratio)
{
    ...
    if (ratio == os->ratio)
        return;
    if (ratio > os->maxRatio)
        ratio = os->maxRatio;
    if (ratio == 1)
        os->ratio = ratio;
    else if (ratio == 2 || ratio == 4 || ratio == 8 || ratio == 16 || ratio == 32 || ratio == 64) {
        os->ratio       = ratio;
        int idx         = (int)(log2f(os->ratio)) - 1 + os->offset;
        os->numTaps     = __leaf_tablesize_firNumTaps[idx];
        os->phaseLength = os->numTaps / os->ratio;
...
void tCrusher_init(tCrusher* const cr, LEAF* const leaf) { tCrusher_initToPool(cr, &leaf->mempool); }
