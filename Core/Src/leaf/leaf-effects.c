...
// it's set to max out at 0.0005 of sample rate (if you don't go above 1.0f in the quality setting) == at 48000
// that's 24. -JS
...
void tTalkbox_update(tTalkbox* const voc) /// update internal parameters...
{
    ...
    int32_t n = (int32_t)(0.01633f * fs); // this sets the window time to 16ms if the buffer is large enough. Buffer
                                          // needs to be at least 784 samples at 48000
    if (n > v->bufsize)
        n = v->bufsize;
...
void tTalkbox_warpedAutocorrelate(float* x, double* dl, double* Rt, unsigned int L, float* R, unsigned int P,
                                  float lambda)
{
    double r1, r2, r1t;
    R[0]  = 0;
...
float tTalkbox_tick(tTalkbox* const voc, float synth, float voice)
{
    ...
    if (v->K++) {
        ...
        w           = v->window[p0];
        fx          = v->buf0[p0] * w;
        v->buf0[p0] = x * w; // 50% overlapping hanning windows
        if (++p0 >= v->N) {
            tTalkbox_lpc(v->buf0, v->car0, v->dl, v->Rt, v->N, v->O, v->warpFactor, v->warpOn, v->k, v->freeze, &v->G);
            p0 = 0;
        }
        ...
        v->buf1[p1] = x * w;
        if (++p1 >= v->N) {
            tTalkbox_lpc(v->buf1, v->car1, v->dl, v->Rt, v->N, v->O, v->warpFactor, v->warpOn, v->k, v->freeze, &v->G);
            p1 = 0;
        }
    }
                  ...
                  float* k, int freeze, float* G)
{
    ...
    if (!freeze) {
        ...
        // in mda's code it's .995 but in Harma's papers I've seen 0.998.  just needs to be less than 1 it seems but
        // maybe some wiggle room to avoid instability from floating point precision -JS
        for (i = 0; i <= o; i++) {
            if (k[i] > 0.998f)
                k[i] = 0.998f;
            else if (k[i] < -0.998f)
...
// find the order-P autocorrelation array, R, for the sequence x of length L and warping of lambda
// wAutocorrelate(&pfSrc[stIndex],siglen,R,P,0);
void tTalkboxFloat_warpedAutocorrelate(float* x, float* dl, float* Rt, unsigned int L, float* R, unsigned int P,
                                       float lambda)
{
    float r1, r2, r1t;
    R[0]  = 0;
...
float tTalkboxFloat_tick(tTalkboxFloat* const voc, float synth, float voice)
{
    ...
    if (v->K++) {
        ...
        v->buf0[p0] = x * w; // 50% overlapping hanning windows
        if (++p0 >= v->N) {
            tTalkboxFloat_lpc(v->buf0, v->car0, v->dl, v->Rt, v->N, v->O, v->warpFactor, v->warpOn, v->k, v->freeze,
                              &v->G);
            p0 = 0;
        }
        ...
        v->buf1[p1] = x * w;
        if (++p1 >= v->N) {
            tTalkboxFloat_lpc(v->buf1, v->car1, v->dl, v->Rt, v->N, v->O, v->warpFactor, v->warpOn, v->k, v->freeze,
                              &v->G);
            p1 = 0;
        }
    }
...
void tTalkboxFloat_lpc(float* buf, float* car, float* dl, float* Rt, int32_t n, int32_t o, float warp, int warpOn,
                       float* k, int freeze, float* G)
{
    float z[ORD_MAX], r[ORD_MAX], x;
    ...
    if (!freeze) {
        ...
        // this is for stability to keep reflection coefficients inside the unit circle
        // but in Harma's papers I've seen 0.998.  just needs to be less than 1 it seems but maybe some wiggle room to
        // avoid instability from floating point precision -JS
        for (int i = 0; i <= o; i++) {
            if (k[i] > 0.998f)
                k[i] = 0.998f;
            else if (k[i] < -0.998f)
...
void tVocoder_init(tVocoder* const voc, LEAF* const leaf) { tVocoder_initToPool(voc, &leaf->mempool); }
...
#else
    if (fabs(v->f[0][11]) < 1.0e-10)
        v->f[0][11] = 0.0f; // catch HF envelope denormal

    for (i = 1; i < nb; i++)
        if (fabs(v->f[i][3]) < 1.0e-10 || fabs(v->f[i][7]) < 1.0e-10)
            for (k = 3; k < 12; k++)
                v->f[i][k] = 0.0f; // catch reson & envelope denormals
#endif
    if (fabs(o) > 10.0f)
        tVocoder_suspend(voc); // catch instability
...
void tVocoder_suspend(tVocoder* const voc)
{
    ...
    for (i = 0; i < v->nbnd; i++)
        for (j = 3; j < 12; j++)
            v->f[i][j] = 0.0f; // zero band filters and envelopes
    v->kout = 0.0f;
    v->kval = 0;
}
...
void tRosenbergGlottalPulse_setOpenLengthAndPulseLength(tRosenbergGlottalPulse* const gp, float openLength,
                                                        float pulseLength)
{
    _tRosenbergGlottalPulse* g       = *gp;
    g->openLength                    = openLength;
...
// init
void tSOLAD_init(tSOLAD* const wp, int loopSize, LEAF* const leaf) { tSOLAD_initToPool(wp, loopSize, &leaf->mempool); }
...
void tSOLAD_ioSamples(tSOLAD* const wp, float* in, float* out, int blocksize)
{
    ...
    while (n--)
        w->delaybuf[i++] = tHighpass_tick(&w->hp, *in++); // copy one input block to delay buffer
...
void tSOLAD_setPeriod(tSOLAD* const wp, float period)
{
    ...
    if (period > maxPeriod)
        period = maxPeriod;
    if (period > MINPERIOD)
        w->period = period; // ignore period when too small
}
...
static void pitchdown(_tSOLAD* const w, float* out)
{
    ...
    if (pitchfactor > 0.5)
        xfadespeed = pitchfactor;
    else
        xfadespeed = 1 - pitchfactor;
    ...
    while (n--) {
        if (readlag > period) // check if read pointer may jump forward...
        {
            if (xfadevalue <= 0) // ...but do not interrupt crossfade
            {
                ...
                while ((jump * 2) < readlag)
                    jump *= 2;   // use available space
...
void tSimpleRetune_init(tSimpleRetune* const rt, int numVoices, float minInputFreq, float maxInputFreq, int bufSize,
                        LEAF* const leaf)
{
    tSimpleRetune_initToPool(rt, numVoices, minInputFreq, maxInputFreq, bufSize, &leaf->mempool);
}

void tSimpleRetune_initToPool(tSimpleRetune* const rt, int numVoices, float minInputFreq, float maxInputFreq,
                              int bufSize, tMempool* const mp)
{
    _tMempool* m      = *mp;
...
void tSimpleRetune_setMode(tSimpleRetune* const rt, int mode)
{
    ...
    if (mode > 0)
        r->shiftFunction = &tPitchShift_shiftTo;
    else
        r->shiftFunction = &tPitchShift_shiftBy;
}
...
void tRetune_init(tRetune* const rt, int numVoices, float minInputFreq, float maxInputFreq, int bufSize,
                  LEAF* const leaf)
{
    tRetune_initToPool(rt, numVoices, minInputFreq, maxInputFreq, bufSize, &leaf->mempool);
}

void tRetune_initToPool(tRetune* const rt, int numVoices, float minInputFreq, float maxInputFreq, int bufSize,
                        tMempool* const mp)
{
    _tMempool* m = *mp;
