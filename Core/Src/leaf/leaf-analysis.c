...
float tEnvelopeFollower_tick(tEnvelopeFollower* const ef, float x)
{
    ...
    if ((x >= e->y) && (x > e->a_thresh))
        ...
        // ef->y = envelope_pow[(uint16_t)(ef->y * (float)UINT16_MAX)] * ef->d_coeff; //not quite the right behavior -
        // too much loss of precision? ef->y = powf(ef->y, 1.000009f) * ef->d_coeff;  // too expensive
...
// returns proportion of zero crossings within window size (0.0 would be none in window, 1.0 would be all zero
// crossings)
...
void tEnvPD_processBlock(tEnvPD* const xpd, float* in)
{
    ...
    t_sample* sump;
    in += n;
    for (count = x->x_phase, sump = x->x_sumbuf; count < x->x_npoints; count += x->x_realperiod, sump++) {
    ...
    if (x->x_phase < 0) {
        ...
        for (count = x->x_realperiod, sump = x->x_sumbuf; count < x->x_npoints; count += x->x_realperiod, sump++)
            sump[0] = sump[1];
        sump[0]    = 0.0f;
...
void tSNAC_ioSamples(tSNAC* const snac, float* in, int size)
{
    ...
    if (!(timeindex & (s->framesize / s->overlap - 1)))
        snac_analyzeframe(snac);
...
static void snac_normalize(tSNAC* const snac)
{
    ...
    for (n = 1, m = s->timeindex + 1; n < seek; n++, m++) {
        ...
        signal2 = inputbuf[(framesizeplustimeindex - n)
                           & mask]; // could this be switched to float resolution without issue? -JS
...
static void snac_periodandfidelity(tSNAC* const snac)
{
    ...
    if (s->periodindex) {
        periodlength = (float)s->periodindex + interpolate3phase(s->processbuf, s->periodindex);
        if (periodlength < 8)
            periodlength = snac_spectralpeak(snac, periodlength);
...
void tZeroCrossingCollector_initToPool(tZeroCrossingCollector* const zc, int windowSize, float hysteresis,
                                       tMempool* const mp)
{
    _tMempool* m               = *mp;
...
static inline void sub_collector_init(_sub_collector* collector, tZeroCrossingCollector* const crossings, float pdt,
                                      int range);
                                                 ...
                                                 float incoming_period)
{
    if (fabsf(incoming_period - collector->_first_period) < collector->_periodicity_diff_threshold) {
        ...
        // periodicity ...
        if (info._periodicity > collector->_fundamental._periodicity && harmonic != collector->_fundamental._harmonic) {
...
int tDualPitchDetector_tick(tDualPitchDetector* const detector, float sample)
{
    ...
    if (ready) {
        ...
        if (!pd2_indeterminate && period != 0.0f) {
            ...
            else if (pd2_diff < p->_mean * 0.03125f)
                i = _i2;
            else if (pd1_diff < p->_mean * 0.03125f)
                i = _i1;
            // Now filter out lower fidelity stuff
            else if (_i1.periodicity < p->thresh)
                return ready;
            // Changing up (bacf tends to lead changes)
            else if ((_i1.frequency > p->_mean && _i2.frequency > p->_mean) && (_i1.frequency < _i2.frequency)
                     && (_i2.periodicity > p->thresh)) {
                if (roundf(_i2.frequency / _i1.frequency) > 1)
                    i = _i1;
                else
                    i = _i2;
            }
            // Changing down
                     ...
                     && (_i2.periodicity > p->thresh)) {
                if (roundf(_i1.frequency / _i2.frequency) > 1)
                    i = _i1;
                else
                    i = _i2;
            }
