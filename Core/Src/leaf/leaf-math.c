...
// This is a fast approximation to log2() found on http://openaudio.blogspot.com/2017/02/faster-log10-and-pow.html
// credited to this post
// https://community.arm.com/developer/tools-software/tools/f/armds-forum/4292/cmsis-dsp-new-functionality-proposal/22621#22621
...
{
    return (0.1640425613334452f * x * x * x) + (-1.098865286222744f * x * x) + (3.148297929334117f * x)
           + -2.213475204444817f;
}
...
// from Heng Li, a combination of inverse square root (see wiki) and inversion:
...
/*
 you pass in a float array to get back two indexes representing the volumes of the left (index 0) and right (index 1)
 channels when t is -1, volumes[0] = 0, volumes[1] = 1 when t = 0, volumes[0] = 0.707, volumes[1] = 0.707 (equal-power
 cross fade) when t = 1, volumes[0] = 1, volumes[1] = 0
 */
...
float LEAF_chebyshevT(float in, int n)
{
    if (n == 0)
        ...
        return 2.0f * in * LEAF_chebyshevT(in, n - 1) - LEAF_chebyshevT(in, n - 2);
}
...
float LEAF_frequencyToMidi(float f) { return (69.0f + 12.0f * log2f(f * INV_440)); }
...
// Adapted from MusicDSP: http://www.musicdsp.org/showone.php?id=238
float LEAF_tanhNoClip(float x) { return x * (27.0f + x * x) / (27.0f + 9.0f * x * x); }
...
{
    return x * (2027025.0f + 270270.0f * x * x + 6930.0f * x * x * x * x + 36.0f * x * x * x * x * x * x)
           / (2027025.0f + 945945.0f * x * x + 51975.0f * x * x * x * x + 630.0f * x * x * x * x * x * x
              + x * x * x * x * x * x * x * x);
}
...
float LEAF_midiToFrequency(float f)
{
    if (f <= -1500.0f)
        ...
        return (LEAF_midiToFrequency(1499.0f));
    else
        return (powf(2.0f, (f - 69.0f) * 0.083333333333333f) * 440.0f);
}
...
float fast_mtof(float f) { return (8.17579891564f * fastexpf(0.0577622650f * f)); }

float faster_mtof(float f) { return (8.17579891564f * fasterexpf(0.0577622650f * f)); }

float ftom(float f) { return (f > 0 ? 17.3123405046f * logf(.12231220585f * f) : -1500.0f); }
...
float atodb(float a) { return 20.0f * log10f(a); }
...
{
    // return powf(10.0f, db * 0.05f);
    return expf(0.115129254649702f
                * db); // faster version from http://openaudio.blogspot.com/2017/02/faster-log10-and-pow.html
}

float fasterdbtoa(float db)
{
    // return powf(10.0f, db * 0.05f);
    return fasterexpf(0.115129254649702f
                      * db); // faster version from http://openaudio.blogspot.com/2017/02/faster-log10-and-pow.html
}


float maximum(float num1, float num2) { return (num1 > num2) ? num1 : num2; }

float minimum(float num1, float num2) { return (num1 < num2) ? num1 : num2; }
...
float median3f(float a, float b, float c) { return fmax(fmin(a, b), fmin(fmax(a, b), c)); }

#if LEAF_INCLUDE_MINBLEP_TABLES
/// MINBLEPS
// https://github.com/MrBlueXav/Dekrispator_v2 blepvco.c
#ifdef ITCMRAM
void __attribute__((section(".itcmram"))) __attribute__((aligned(32)))
