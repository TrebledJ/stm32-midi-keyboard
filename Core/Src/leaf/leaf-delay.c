...
float tDelay_tick(tDelay* const dl, float input)
{
    ...
    if (++(d->inPoint) == d->maxDelay)
        d->inPoint = 0;

    // Output
    d->lastOut = d->buff[d->outPoint];
    if (++(d->outPoint) == d->maxDelay)
        d->outPoint = 0;
...
void tDelay_setDelay(tDelay* const dl, uint32_t delay)
{
    ...
    // read chases write
    if (d->inPoint >= delay)
        d->outPoint = d->inPoint - d->delay;
    else
        d->outPoint = d->maxDelay + d->inPoint - d->delay;
}
...
float tHermiteDelay_tick(tHermiteDelay* const dl, float input)
{
    ...
    uint32_t idx = (uint32_t)d->outPoint;
    d->lastOut =
        LEAF_interpolate_hermite_x(d->buff[((idx - 1) + d->maxDelay) & d->bufferMask], d->buff[idx],
                                   d->buff[(idx + 1) & d->bufferMask], d->buff[(idx + 2) & d->bufferMask], d->alpha);
...
float tHermiteDelay_tapOutInterpolated(tHermiteDelay* const dl, uint32_t tapDelay, float alpha)
{
    ...
    return LEAF_interpolate_hermite_x(d->buff[((idx - 1) + d->maxDelay) & d->bufferMask], d->buff[idx],
                                      d->buff[(idx + 1) & d->bufferMask], d->buff[(idx + 2) & d->bufferMask], alpha);
}
...
void tAllpassDelay_setDelay(tAllpassDelay* const dl, float delay)
{
    ...
    while (outPointer < 0)
        outPointer += d->maxDelay; // mod max length
...
float tTapeDelay_tick(tTapeDelay* const dl, float input)
{
    ...
    d->lastOut = LEAF_interpolate_hermite_x(d->buff[((idx - 1) + d->maxDelay) % d->maxDelay], d->buff[idx],
                                            d->buff[(idx + 1) % d->maxDelay], d->buff[(idx + 2) % d->maxDelay], alpha);
...
float tTapeDelay_tapOut(tTapeDelay* const dl, float tapDelay)
{
    ...
    // Check for wraparound.
    while (tap < 0.f)
        tap += (float)d->maxDelay;

    int idx = (int)tap;

    float alpha = tap - idx;

    float samp = LEAF_interpolate_hermite_x(d->buff[((idx - 1) + d->maxDelay) % d->maxDelay], d->buff[idx],
                                            d->buff[(idx + 1) % d->maxDelay], d->buff[(idx + 2) % d->maxDelay], alpha);
