...
/*!
 ...
 @details Buffer for use in Sampler objects. Can selectively record audio and keeps track of the length of recorded
 audio.
 @{
 ...
 @param sampler A pointer to the relevant tBuffer.
 @param mode The new mode, either RecordOneShot to record one buffer length or RecordLoop to record on loop, overwriting
 old samples.
...
typedef enum RecordMode { RecordOneShot = 0, RecordLoop, RecordModeNil } RecordMode;
...
typedef enum PlayMode { PlayNormal, PlayLoop, PlayBackAndForth, PlayModeNil } PlayMode;
