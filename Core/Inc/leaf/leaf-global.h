...
struct LEAF {
    ...
    int clearOnAllocation; //!< A flag that determines whether memory allocated from the LEAF memory pool will be
                           //!< cleared.
    ...
    void (*errorCallback)(
        LEAF* const, LEAFErrorType); //!< A pointer to the callback function for LEAF errors. Can be set by the user.
