...
__ALIGN_BEGIN static uint8_t USBD_MIDI_CfgDesc[USB_MIDI_CONFIG_DESC_SIZ] __ALIGN_END = {
    ...
    0x00,       /* bcdADC */
    0x01, 0x09, /* wTotalLength */
    0x00, 0x01, /* binCollecton */
    ...
    HIBYTE(USB_FS_MAX_PACKET_SIZE), 0x00, /* bInterval: ignore for Bulk transfer */
    0x00,                                 /* bRefresh */
    0x00,                                 /* bSynchAddress */
...
static uint8_t USBD_MIDI_Init(USBD_HandleTypeDef* pdev, uint8_t cfgidx)
{
    ...
    if (pdev->pClassData == NULL) {
        ...
        USBD_LL_PrepareReceive(pdev, MIDI_OUT_EP, hmidi->rx_buffer, USB_FS_MAX_PACKET_SIZE);
    }
    return USBD_OK;
}
 ...
 * @retval status
 */
static uint8_t USBD_MIDI_Setup(USBD_HandleTypeDef* pdev, USBD_SetupReqTypedef* req) { return USBD_OK; }
 ...
 * @retval status
 */
static uint8_t USBD_MIDI_EP0_RxReady(USBD_HandleTypeDef* pdev) { return USBD_OK; }
/**
 ...
 * @retval status
 */
static uint8_t USBD_MIDI_SOF(USBD_HandleTypeDef* pdev) { return USBD_OK; }
 ...
 * @retval status
 */
void USBD_MIDI_Sync(USBD_HandleTypeDef* pdev) { return; }
 ...
 * @retval status
 */
static uint8_t USBD_MIDI_IsoINIncomplete(USBD_HandleTypeDef* pdev, uint8_t epnum) { return USBD_OK; }
/**
 ...
 * @retval status
 */
static uint8_t USBD_MIDI_IsoOutIncomplete(USBD_HandleTypeDef* pdev, uint8_t epnum) { return USBD_OK; }
/**
 ...
 * @retval status
 */
static void MIDI_REQ_GetCurrent(USBD_HandleTypeDef* pdev, USBD_SetupReqTypedef* req) { return; }
 ...
 * @retval status
 */
static void MIDI_REQ_SetCurrent(USBD_HandleTypeDef* pdev, USBD_SetupReqTypedef* req) { return; }
 ...
 * @retval status
 */
uint8_t USBD_MIDI_SetTxBuffer(USBD_HandleTypeDef* pdev, uint8_t* buff, uint16_t length)
{
...
uint8_t USBD_MIDI_TransmitPacket(USBD_HandleTypeDef* pdev)
{
    ...
    if (hmidi->tx_busy == 0) {
        ...
        /* Transmit */
        USBD_LL_Transmit(pdev, MIDI_IN_EP, hmidi->tx_buffer, (uint8_t)hmidi->tx_length);
